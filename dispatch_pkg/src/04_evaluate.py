...
def evaluate(model, env, name, eval_steps=500, seed=43):
    ...
    cost_total_dollar = (cost_genset_total + cost_wind_total + cost_carbon_total
...
def main():
    ...
    if bl:
        ...
        md.append("| Agent | Reward | Total Cost ($) | Genset Cost | Carbon Cost | CO2 (t) | Wind Util (%) | Imbalance (MWh) |")
        md.append("|:------|-------:|---------------:|------------:|------------:|--------:|--------------:|----------------:|")
        for m in metrics_list:
            md.append(f"| {m['name']} | {m['total_reward']:,.0f} | "
                      f"{m['total_cost_dollar']:,.0f} | "
                      f"{m['cost_genset']:,.0f} | {m['cost_carbon']:,.0f} | "
